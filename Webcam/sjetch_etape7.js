...
function preload(){
    console.log("Monde Ouvert etape 7");
        ...
        for( let col = 0; col < w; col += 1){
            ...
            if (color_select) { // si on a choisit une couleur (avec un click de souris), alors...
                ...
                if (d_calc < d_max) { // si la distance calculée est inférieure à d_max...
                    img_new.pixels[i + 0] = img.pixels[i + 0]; // composante rouge de l'image de fond
                    img_new.pixels[i + 1] = img.pixels[i + 1]; // composante verte de l'image de fond
                    img_new.pixels[i + 2] = img.pixels[i + 2]; // composante bleue de l'image de fond
                    img_new.pixels[i + 3] = img.pixels[i + 3]; // composante alpha de l'image de fond
                } else { // sinon...
